        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and pay_form.is_valid():
                ...
                if pay_method.payment_option == 'Cash on delivery':
                    ...
                    return HttpResponseRedirect(reverse('index'))  # Redirect to index page
...
#SSL Commerce  
                    ...
                    order_qs=Order.objects.filter(user=request.user, ordered=False)
                    order_item=order_qs[0].cart_product.all()
                    order_item_count=order_qs[0].cart_product.count()
                    order_total=order_qs[0].get_total()
                    mypayment.set_product_integration(total_amount=Decimal(order_total), currency='BDT', product_category='Food', product_name=order_item, num_of_item=order_item_count, shipping_method='YES', product_profile='None')

#customer profile information
                    current_user=request.user
                    mypayment.set_customer_info(name=current_user.username, email=email, address1=address, address2='demo2', city='dhaka', postcode=1237, country='Bangladesh', phone=phone)
